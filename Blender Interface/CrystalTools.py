...
#from mathutils.Matrix import Rotation
...
def makeblendermesh(name, PointList, EdgeList = None, FaceList = None):
    ...
    return Mesh
#
def makeblendercachedob(name,Mesh):
...
def makeblendermeshob(name, PointList, EdgeList = None, FaceList = None):
    ...
    #bpy.context.active_object = mesh_object
...
def ray_to_plane(ray_point, ray_vector, plane_point, normal_vector,
    ...
    ray_vector is the direction and distance of the ray
    plane_point is any point on the plane
    normal_vector is the normal vector of the plane
        all variables should be instances of the Vector class
...
def n_gon(point_list):
    ''' Return a list of faces comprising an n-gon.
    
    point_list is a list of the indexes of the points that
    should comprise the n-gon, in the order desired.
...
class Solid(object):
    '''Primary class for solid geometry
    '''
    # Override these globals with subclassing
    # label for the crystal family
    label = "A Solid"            
    
    def create(self):
        # list of points, should not be vectors! [(x, y, z), ...]
        points = []
        # list of faces, 3 or more points each. Use point indicies.
        faces = []
        # localize some instance and class variables for ease of reference
        planes = self.planes
        ...
        for this_info in planes_to_make:
            ...
            if these_points is None:
                ...
                return None
            point_indicies = [i for i in range(len(points), 
                                               len(points) + len(these_points))]
            ...
            points += these_points
            # process all the planes that were found
        ...
        ob = makeblendermeshob(self.label, points, FaceList = faces)
        # Note the faces that were never used
        used_planes = [i[0] for i in planes_to_make]
        extra_planes = []
        for i in range(len(planes)):
            if i not in used_planes:
                extra_planes += [i]
        print("un-used planes", extra_planes)
        # store the extra planes, in case we want to use them for something
        # at a later time.
        self.extra_planes = extra_planes
        return ob
    
    def __init__(self, name, face_locations, radius = None):
        # a list of vectors from the center to the plane, along the normal
        self.planes = []
        self.label = name
        for point in face_locations:
            cur_point = Vector(point)
            self.planes += [cur_point]
        # the maximum radius of any point. Currently never used.
        self.radius = radius

def make_solid_from_points(points):
    new_solid = Solid("From_Points", points)
    origin = Vector((0,0,0))
    rot_axis = Vector((0,0,10))
    ob = new_solid.create()
    return ob, new_solid
...
def make_random_solid_from_points(points, rand_magnitude):
    ...
    #make the solid
    ob, new_solid = make_solid_from_points(random_points)
    return ob
...
def make_solid_from_figure(source_ob = None):
    ...
    ob, new_solid = make_solid_from_points(points)
    ...
    # clean out the un-necessary figure points
    # doesn't work... need an elegant way to do this.
    #bad_points = new_solid.extra_planes
    #bad_points.sort(reverse=True)
    #for idx in bad_points:
    #    source_ob.data.vertices.pop(idx)
    return ob

def make_figure_from_Mesh(source_ob = None):
    # Create a definition figure from an existing mesh object
    if source_ob == None:
        source_ob = bpy.context.active_object
    # working bmesh
    bm = bmesh.new()
    ...
    ob = makeblendermeshob("Generated Figure", figure_points, EdgeList = [(0,1)])
    ob.location = source_ob.location
    ob.rotation_euler = source_ob.rotation_euler
    ob.scale = source_ob.scale
    return ob
...
def move_center(source_ob = None, new_center = None):
    '''Move the plane figure center to the position of the last created point
    '''
    # So that there are some edges to click on
    AllEdges = [(0,1),(1,2),(2,3),(0,2),(0,3),(1,3)]
    if source_ob == None:
        source_ob = bpy.context.active_object
    points_raw = source_ob.data.vertices
    source_points = [i.co for i in points_raw]
    # I used to use the cursor to re-position the center
    #cursor = bpy.context.scene.cursor_location
    # but now:
    if new_center is None:
        # use the last point created as the new center
...
def make_cluster_from_figure(num_of_chunks, dupe_per_chunk):
    '''Generate a group of similar objects
